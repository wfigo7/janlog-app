...
**ユーザーストーリー:** 管理者として、麻雀ルールをマスタデータとして管理したいので、異なるルールでの対局に対応できる

#### 受入基準

1. WHEN 管理者がルール管理画面にアクセスした時 THEN システムはルール一覧と新規作成機能を表示する
2. WHEN 管理者が新しいルールを作成する時 THEN システムはルール名、ゲームモード（3人/4人）、ウマ、オカの入力欄を表示する
3. WHEN 管理者がルール情報を保存する時 THEN システムは入力データを検証してDynamoDBに保存する
4. WHEN 管理者がルールを編集・削除する時 THEN システムは対応する操作を実行する
5. WHEN 一般ユーザーがルール管理機能にアクセスしようとした時 THEN システムは「管理者権限が必要です」メッセージを表示する
